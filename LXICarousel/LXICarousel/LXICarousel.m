    ...
    if (offset <= 0) {
        switch (_firstCardType)
        {
            ...
            case iCarouselTypeInvertedTimeMachine:
            {
                ...
                CGFloat spacing = [self valueForOption:iCarouselOptionSpacing withDefault:0.8];
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    //calculate transform
#pragma mark 替换原有的transform
//    CATransform3D transform = [self transformForItemViewWithOffset:offset];
    CATransform3D transform = [self newTransformForItemViewWithOffset:offset];
